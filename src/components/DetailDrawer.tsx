...
export function DetailDrawer({
  ...
  if (!item && brief) {
    return (
      <aside className="drawer brief-drawer">
        <div className="drawer-toolbar">
          <span>Daily brief</span>
          <span className="brief-drawer-date">{dayjs(brief.createdAt).format("MMM D, h:mm A")}</span>
        </div>
        <div className="drawer-content brief-drawer-content">
          <div className="brief-drawer-title">
            <span className="copilot-mark" aria-hidden="true">C</span>
            <div><span className="eyebrow">Generated by Copilot</span><h2>Daily summary</h2></div>
          </div>
          <div className="brief-content"><BriefContent markdown={brief.markdown} /></div>
          {brief.conversationUrl && <a className="open-source" href={brief.conversationUrl} target="_blank" rel="noreferrer">Open generated Copilot chat <span>↗</span></a>}
        </div>
      </aside>
    );
  }
  ...
  return (
    <aside className="drawer">
      <div className="drawer-toolbar">
        ...
        <button className="close" type="button" onClick={onClose} aria-label={brief ? "Show daily brief" : "Close details"}>{brief ? "←" : "×"}</button>
      </div>
